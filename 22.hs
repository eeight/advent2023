...
solve bs = let
    ...
    counts = A.accumArray (+) 0 (0, maxn) $ map ((,1) . snd) supportP
    supportM = M.fromListWith (++) $ [(n, []) | n <- [0..maxn]] ++ map (\(a, b) -> (a, [b])) supportP
    in length $ filter (\(_, ns) -> all (> 1) (map (counts A.!) ns)) $ M.toList supportM
