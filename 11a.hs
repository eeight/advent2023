...
solve :: A.Array (Int, Int) Char -> Integer
solve f = let
    (_, (n1, m1)) = A.bounds f
    ...
    expand s i = i + (1000000 - 1) * (S.size $ fst $ S.split i s)

    galaxyCoords = map (\(y, x) -> (expand emptyRows y, expand emptyCols x)) . filter ((== '#') . (f A.!)) $ A.indices f

    in sum [fromIntegral $ abs (x1 - x2) + abs (y1 - y2) | ((y1, x1):gs) <- tails galaxyCoords, (y2, x2) <- gs]
